...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      ...
      <dl class="am-icon-home" style="float: right;"> 当前位置： 首页 > 用户管理</a></dl>
  ...
  <ul>
    ...
    <form class="am-form am-g" method="post" action="<%=path %>/user/findBy.do?page=1&size=5">
    <li><input type="text" name="userId" class="am-form-field am-input-sm am-input-xm" placeholder="用户ID" /></li>
    <li><input type="text" name="userName" class="am-form-field am-input-sm am-input-xm" placeholder="用户名" /></li>
    ...
    <form class="am-form am-g" action="<%=path %>/user/delete.do" method="post">
           <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
		          ...
		          <%
				    ...
				    <tr>
						...
						<td><div class="am-btn-toolbar">
			                 <div class="am-btn-group am-btn-group-xs">
			                 <a href="<%=path %>/user/findById.do?userId=<%= user.getUserId() %>&type=1" class="am-btn am-btn-default am-btn-xs am-text-success am-round am-icon-file" data-am-modal="{target: '#my-popups'}" title="修改用户权限"></a>
			                  </div>
			                 </div>
			             </td>
				    </tr>
            ...
            <ul class="am-pagination am-fr">
                <li><a href="<%=path %>/user/findBy.do?userId=<%=userId %>&userName=<%=userName %>&page=1&size=5&type=1">«首页</a></li>
                <li><a href="<%=path %>/user/findBy.do?userId=<%=userId %>&userName=<%=userName %>&page=<%=nowPage-1 %>&size=5&type=1">上一页</a></li>
                <li><a href="<%=path %>/user/findBy.do?userId=<%=userId %>&userName=<%=userName %>&page=<%=nowPage+1 %>&size=5&type=1">下一页</a></li>
                <li><a href="<%=path %>/user/findBy.do?userId=<%=userId %>&userName=<%=userName %>&page=<%=maxPage %>&size=5&type=1">末页»</a></li>
             </ul>
          <hr />
          <p>注：.....</p>
        </form>
