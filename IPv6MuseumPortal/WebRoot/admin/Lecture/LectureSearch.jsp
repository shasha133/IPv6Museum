...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      ...
      <dl class="am-icon-home" style="float: right;"> 当前位置： 首页 ><a href="<%=path %>/Activity/list.do?p=1&size=5&type=2">专题讲座</a></dl>
      
      <dl>
        <button type="button" href="<%=path %>/admin/Lecture/saveLecture.jsp" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus"> 发布活动</button>
      </dl>
    ...
    <form class="am-form am-g" action="<%=path %>/Activity/delete.do" method="post">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
		           ...
		           <%
				    ...
				    <tr>
						...
						<td><div class="am-btn-toolbar">
			                 <div class="am-btn-group am-btn-group-xs">
			                   	<a href="<%=path %>/Activity/findById.do?activityId=<%=activity.getActivityId() %>&type=2" class="am-btn am-btn-default am-btn-xs am-text-success am-round am-icon-file" data-am-modal="{target: '#my-popups'}" title="编辑"></a>
			                 </div>
			                 </div>
			             </td>
				    </tr>
            ...
            <ul class="am-pagination am-fr">
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=1&size=5&type=2">«首页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=nowPage-1 %>&size=5&type=2">上一页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=nowPage+1 %>&size=5&type=2">下一页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=maxPage %>&size=5&type=2">末页»</a></li>
             </ul>
          <hr />
          <p>注：.....</p>
        </form>
