...
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
	...
	String material=String.valueOf(request.getAttribute("Material"));
	String dynasty=String.valueOf(request.getAttribute("Dynasty"));
	String apply=String.valueOf(request.getAttribute("Apply"));
	String value=String.valueOf(request.getAttribute("Value"));
	String exhibitsName=String.valueOf(request.getAttribute("exhibitsName"));
	String i=String.valueOf(request.getAttribute("maxPage"));
	String j=String.valueOf(request.getAttribute("page"));
	int material1=Integer.valueOf(material);
    int apply1=Integer.valueOf(apply);
    int value1=Integer.valueOf(value);
    int dynasty1=Integer.valueOf(dynasty);
...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      ...
      <dl class="am-icon-home" style="float: right;"> 当前位置： 首页 > <a href="<%=path %>/Exhibits/list.do?p=1&size=5">展品信息</a> </a></dl>
...
<form class="am-form am-g" method="post" action="<%=path %>/Exhibits/findBy.do?page=1&size=5">
  ...
  <ul>
    ...
    <li><input type="text" name="exhibitsName" class="am-form-field am-input-sm am-input-xm" placeholder="<%= exhibitsName %>" /></li>
            ...
            <tbody>
              <%
				    ...
				    <tr>
				        ...
				        <td><a href="<%=path %>/Exhibits/findById.do?exhibitsId=<%=exhibits.getExhibitsId() %>?type=1"><%= exhibits.getExhibitsId()%></a></td>
             ...
             <ul class="am-pagination am-fr">
                <li><a href="<%=path %>/Exhibits/findBy.do?Material=<%=material %>&Dynasty=<%=dynasty %>&Apply=<%=apply %>&Value=<%=value %>&exhibitsName=<%=exhibitsName %>&page=1&size=5&type=2">«首页</a></li>
                <li><a href="<%=path %>/Exhibits/findBy.do?Material=<%=material %>&Dynasty=<%=dynasty %>&Apply=<%=apply %>&Value=<%=value %>&exhibitsName=<%=exhibitsName %>&page=<%=nowPage-1 %>&size=5&type=2">上一页</a></li>
                <li><a href="<%=path %>/Exhibits/findBy.do?Material=<%=material %>&Dynasty=<%=dynasty %>&Apply=<%=apply %>&Value=<%=value %>&exhibitsName=<%=exhibitsName %>&page=<%=nowPage+1 %>&size=5&type=2">下一页</a></li>
                <li><a href="<%=path %>/Exhibits/findBy.do?Material=<%=material %>&Dynasty=<%=dynasty %>&Apply=<%=apply %>&Value=<%=value %>&exhibitsName=<%=exhibitsName %>&page=<%=maxPage %>&size=5&type=2">末页»</a></li>
             </ul>
