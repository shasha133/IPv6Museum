...
%>
<%
	List<Activity> list=(List<Activity>)request.getAttribute("SearchList");
	String activitytitle=String.valueOf(request.getAttribute("activitytitle"));
	String activitytime=String.valueOf(request.getAttribute("activitytime"));
	String activityId=String.valueOf(request.getAttribute("activityId"));
 ...
 <font size=3 >   
    ...
    <div class="sideMenu">
	  ...
	  <h3 class="am-icon-gears"><em></em> 统计管理</h3>
		   ...
		   <ul><li><a href="<%=path %>/News/set.do?User_Id=<%=id %>&User_root=<%=root%>">返回主页</a></li>	
		  </ul>
    </div>
    </div>
  </font>	      
...
<div class="admin-biaogelist">
    ...
    <div class="listbiaoti am-cf">
      <ul class="am-icon-flag on"> 展厅活动管理</ul>
      
      <dl class="am-icon-home" style="float: right;"> 当前位置： 首页 ><a href="<%=path %>/Activity/list.do?p=1&size=5&type=1">展厅活动</a></dl>
      
      <dl>
       <a href="<%=path %>/admin/Activity/saveActivity.jsp" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus"> 发布活动</a></button>
     </dl>
  ...
  <ul>
   ...
   <form class="am-form am-g" method="post" action="<%=path %>/Activity/findBy.do?page=1&size=5&type=1">
    ...
    <form class="am-form am-g" action="<%=path %>/Activity/delete.do" method="post">
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            ...
            <tbody>
		           <%
		             ...
		             %> 
				    <tr>
				        <td><input type="checkbox" name="id" value="<%= activity.getActivityId()%>" /></td>
				        <td><%= activity.getActivityId()%></td>
					    <td><%= activity.getActivityTitle()%></td>
						<td><%= activity.getActivityContext()%></td>
						<td><%= activity.getActivityAbstract()%></td>
						<td><%= activity.getActivityTime()%></td>
						<td><%= activity.getActivityTimeStart()%></td>
						<td><%= activity.getActivityTimeEnd()%></td>
						<td><%= activity.getActivityState()%></td> 
						<td><div class="am-btn-toolbar">
			                 <div class="am-btn-group am-btn-group-xs">
			               <!--	<a href="<%=path %>/Activity/top.do?activityId=<%=activity.getActivityId() %>" class="am-btn am-btn-default am-btn-xs am-text-secondary am-round am-icon-file" data-am-modal="{target: '#my-popups'}" title="置顶/取消置顶（前台显示热门活动）"></a> -->
			                  	<a href="<%=path %>/Activity/findById.do?activityId=<%=activity.getActivityId() %>&type=1" class="am-btn am-btn-default am-btn-xs am-text-success am-round am-icon-file" data-am-modal="{target: '#my-popups'}" title="编辑"></a>
			                 </div>
			                 </div>
			             </td>
				    </tr>
            ...
            <ul class="am-pagination am-fr">
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=1&size=5&type=1">«首页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=nowPage-1 %>&size=5&type=1">上一页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=nowPage+1 %>&size=5&type=1">下一页</a></li>
                <li><a href="<%=path %>/Activity/findBy.do?activityId=<%=activityId %>&activitytitle=<%=activitytitle %>&activitytime=<%=activitytime %>&page=<%=maxPage %>&size=5&type=1">末页»</a></li>
             </ul>
          <hr />
          <p>注：.....</p>
        </form>
