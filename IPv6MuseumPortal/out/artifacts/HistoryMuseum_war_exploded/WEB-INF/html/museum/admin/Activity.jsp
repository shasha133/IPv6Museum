...
<div class="am-cf admin-main">
    ...
    <div class="admin-biaogelist">
        ...
        <div class="listbiaoti am-cf">
            ...
            <dl class="am-icon-home" style="float: right;"> 当前位置： 首页 >展厅活动管理</dl>
            <dl>
                <a href="/Activity/ReleaseActivity" class="am-btn am-btn-danger am-round am-btn-xs am-icon-plus">
                    发布活动</a>
            </dl>
        </div>
        ...
        <div class="am-btn-toolbars am-btn-toolbar am-kg am-cf">
            <ul>
                ...
                <form class="am-form am-g" method="post" method="post" action="/Activity/findBy?page=1&size=5&type=1">
                    <li><input type="text" name="activitytitle" class="am-form-field am-input-sm am-input-xm"
                               placeholder="活动标题"/></li>
                    <li><input type="text" name="activitytime" class="am-form-field am-input-sm am-input-xm"
                               placeholder="活动发布时间"/></li>
                    <li><input type="text" name="activityId" class="am-form-field am-input-sm am-input-xm"
                               placeholder="活动编号"/></li>
                    <li>
        ...
        <form class="am-form am-g" method="post" action="/Activity/delete" method="post">
            <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
                ...
                <tr class="am-success">
                    ...
                    <th width="163px" class="table-set">置顶</th>
                ...
                <tr>
                    ...
                    <td><%= activity.getActivityTitle()%>
                    </td>
                    <td><%= activity.getActivityAbstract()%>
                    </td>
                    <td><%= activity.getActivityTime()%>
                    </td>
                    <td><%= activity.getActivityTimeStart()%>
                    </td>
                    <td><%= activity.getActivityTimeEnd()%>
                    </td>
                    <td><%= activity.getActivityState()%>
                    </td>
                    <%
                        if (1 == (activity.getActivityTop())) {
                    %>
                    <td>是</td>
                    <%
                    } else {
                    ...
                    <td>
                        <div class="am-btn-toolbar">
                            <div class="am-btn-group am-btn-group-xs">
                                <a href="/Activity/top?activityId=<%=activity.getActivityId() %>"
                                   class="am-btn am-btn-default am-btn-xs am-text-secondary am-round am-icon-file"
                                   data-am-modal="{target: '#my-popups'}" title="置顶/取消置顶（前台显示热门活动）"></a>
                                <a href="/Activity/findById?activityId=<%=activity.getActivityId() %>&type=1"
                                   class="am-btn am-btn-default am-btn-xs am-text-success am-round am-icon-file"
                                   data-am-modal="{target: '#my-popups'}" title="编辑"></a>
                            </div>
                        </div>
                    </td>
                </tr>
            ...
            <ul class="am-pagination am-fr">
                <li><a href="/Activity/list?p=1&size=5">«首页</a></li>
                <li><a href="/Activity/list?p=<%=nowPage-1 %>&size=5">上一页</a></li>
                <li><a href="/Activity/list?p=<%=nowPage+1 %>&size=5">下一页</a></li>
                <li><a href="/Activity/list?p=<%=maxPage %>&size=5">末页»</a></li>
            </ul>
            <hr/>
            <p>注：.....</p>
        </form>
